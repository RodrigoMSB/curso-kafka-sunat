#!/bin/bash
# ============================================================
# Lab 13 - Recuperación del alumno (95)
...
if [ "$COMPLETO" = "si" ]; then
    ...
    W=0
    until [ "$(curl -sf --max-time 8 http://localhost:8083/connectors/novatech-source-pedidos/status 2>/dev/null | grep -o '"state":"RUNNING"' | grep -c .)" -ge 2 ] || [ "$W" -ge 90 ]; do
        sleep 5; W=$((W+5))
    done
    echo -e "  ${GREEN}connector novatech-source-pedidos creado (RUNNING)${NC}"
fi
