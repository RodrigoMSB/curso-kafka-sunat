...
| **F-04** | D | labs/lab-{07,11,12,13}/plantillas/ (plantillas de validación con el apodo del operador en el nombre) | El apodo del operador aparecía en NOMBRES de archivo trackeados en plantillas ACTIVAS del curso | 4 archivos en plantillas activas (+ 3 en `_archive-28h/`) | **MAYOR** | ✅ **RESUELTO SPEC-31**: movidos a `_fuente-extra/plantillas/reporte-validado-fuente.md`, apodo scrubbeado |
...
| **F-11** | E | plantillas de labs 05,06,07,11,12,13 | Plantillas "VALIDADO" stale duplicadas en `plantillas/` activas (en 01-04 ya movidas en SPEC-26; en 05-13 no) | `git ls-files \| grep VALIDADO` fuera de `_fuente-extra` | **MENOR** | ✅ **RESUELTO SPEC-31**: movidas a `_fuente-extra/plantillas/` |
...
| **F-13** | D | `_archive-28h/` (raíz, 147 archivos trackeados) | El curso viejo de 28h estaba versionado; arrastraba identidad "28h" y nombres con el apodo del operador | `git ls-files _archive-28h \| wc -l` = 147 | **MENOR** | ✅ **RESUELTO SPEC-31**: `git rm -r _archive-28h/` (vive en su propio repo) |
...
|---|---|---|---|
| P-01 | Lab 01 desafío: storage de OTRO cluster-id falla | No reproducible en single-node; escenario multi-nodo | ✅ **VERIFICADA** (arquitecto, ASF/Red Hat): `InconsistentClusterIdException` — «The Cluster ID X doesn't match stored clusterId … in meta.properties» |
| P-02 | Lab 09: Spring Boot 4.1 / spring-kafka 4 / Jackson 3 | Versiones/semántica de frameworks externos | ✅ **VERIFICADA** (arquitecto: Maven Central + blog oficial Spring; ya verificado en SPEC-17) |
| P-03 | Lab 14: excepción exacta `NotEnoughReplicas` | Comportamiento verificado (escritura rechazada con ISR<min.insync); string exacto no aislado | ✅ **VERIFICADA** (arquitecto, wiki Apache/Red Hat): error `NOT_ENOUGH_REPLICAS` / `NotEnoughReplicasException: Messages are rejected since there are fewer in-sync replicas than required` |
...
- **Menciones de IA/generadores en contenido** (D4): cero en el texto de los materiales (los "generado por" hallados son prosa legítima: "archivo generado por la imagen"). *La fuga del apodo es en nombres de archivo, no en contenido* → F-04.
...
## 8. Estado de resolución (SPEC-31)

**Cero afirmaciones falsas en el curso** — las tres PENDIENTE-EXTERNA quedaron VERIFICADAS contra fuentes oficiales (arriba). Fixes aplicados en SPEC-31 con clasificación en firme de Rodrigo:

- **Aseo** (F-04, F-11, F-13): plantillas de validación movidas a `_fuente-extra/` sin el apodo; `_archive-28h/` eliminado; el barrido del apodo del operador quedó en cero en todo el repo.
- **Soluciones incompletas** (F-01, F-02, F-03): respuestas modelo de retención (05), análisis de rendimiento (07) y capstone (14) redactadas; plantilla del 14 expandida a las 60 preguntas de las 6 guías.
- **F-05** reclasificado MENOR y documentado (nota de cobertura repartida en READMEs 01/02).
- **MENOR**: duraciones a 60 min (F-06), los 14 READMEs citan el 90 (F-08), numeración lab-07 (F-09), nota kafbat/Landoop (F-15), nota de identificadores heredados (F-07).

Este documento se conserva como registro histórico de la auditoría (única ubicación donde el apodo del operador se menciona, reemplazado por «el operador»).
