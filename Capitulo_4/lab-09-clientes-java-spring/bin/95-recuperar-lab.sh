#!/bin/bash
# ============================================================
# Lab 09 - Recuperación del alumno (95)
...
if [ "$COMPLETO" = "si" ]; then
    ...
    if command -v mvn >/dev/null 2>&1; then
        ...
        echo -e "  ${GREEN}proyectos cliente-java y cliente-spring compilados${NC}" || \
        echo -e "  ${YELLOW}la compilación falló; revisa mvn a mano${NC}"
    else
        echo -e "  ${YELLOW}mvn no está en el PATH; omito la compilación (instala JDK+Maven para --completo)${NC}"
    fi
fi
...
if bash bin/90-test-lab.sh; then
    ...
    echo -e "${CYAN}  Para producir/consumir: mvn -f cliente-java/pom.xml exec:java -Dexec.mainClass=com.novatech.kafka.ProductorApp -Dexec.args=10 (ver guía 01).${NC}"
    exit 0
else
