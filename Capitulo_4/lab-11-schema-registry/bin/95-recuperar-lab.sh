#!/bin/bash
# ============================================================
# Lab 11 - Recuperación del alumno (95)
...
if [ "$COMPLETO" = "si" ]; then
    ...
    W=0; until curl -sf --max-time 5 http://localhost:8081/subjects >/dev/null 2>&1 || [ "$W" -ge 90 ]; do sleep 4; W=$((W+4)); done
    for id in 1 2 3; do bash kafka-cli/produce-pedido-avro.sh "$id" 1001 "Caja premium" 10 25000.00 pendiente >/dev/null 2>&1; done
    echo -e "  ${GREEN}schema Avro registrado y datos sembrados (novatech.lab10.pedidos)${NC}"
fi
